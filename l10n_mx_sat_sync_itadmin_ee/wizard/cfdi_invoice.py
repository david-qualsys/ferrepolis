...
class CfdiInvoiceAttachment(models.TransientModel):
                ...
                try:
                    if create_so_po:
                        if attachment.cfdi_type=='I':
                            ...
                            res = self.import_purchase_order(attachment)
                        else:
                            continue
                    else: 
                        if attachment.cfdi_type=='I':
                            #record = import_customer_bill_obj.create({'import_file':attachment.datas, 'file_name' : attachment.datas_fname})
                            res = self.import_customer_invoice(attachment,self.journal_id)
                        elif attachment.cfdi_type=='SI':
                            ...
                            res = self.import_supplier_invoice(attachment,self.supplier_journal_id)
                    ...
                    if res and type(res)==dict:
                        ...
                        if res.get('res_model')=='account.invoice':
                            val.update({'invoice_ids':[(6,0, [res.get('res_id')])]})
    ...
    @api.multi
    def import_customer_invoice(self, file_contents, journal=False):
        file_content = file_contents.datas
...
#                raise Warning("Factura ya existente con la referencia del vendedor %s"%(customer_reference))
        ...
        invoice_vals = {
            ...
            #'fecha_certificacion': timbrado_data.get('@FechaTimbrado'), #data.get('Comprobante',{}).get('@FechaTimbrado')
...
#                     tax_ids.append(tax_exist.id)
    ...
    @api.multi
    def import_supplier_invoice(self, file_contents, journal=False):
        #raise Warning(file_contents.datas_fname)
        file_content = file_contents.datas
        ...
        vendor_uuid = timbrado_data.get('@UUID')
        #raise Warning(l10n_mx_edi_cfdi_uuid_cusom)
        ...
        if vendor_reference != '':
            ...
            if invoice_exist:
                vendor_reference = ''
                #Alerta en caso de que exista la referencia del proveedor
        ...
        invoice_vals = {
            ...
            'l10n_mx_edi_usage' : receptor_data.get('@UsoCFDI'),
            'l10n_mx_edi_cfdi_uuid_cusom': vendor_uuid,
            #'l10n_mx_edi_cfdi_name' : file_contents.datas_fname,            
            'l10n_mx_edi_cfdi_xml': file_contents.datas,
            #'import_file':file_content.datas,
            #'file_name' : file_contents.datas_fname,
            #'forma_pago':data.get('Comprobante',{}).get('@FormaPago',{}), 
            #'methodo_pago':data.get('Comprobante',{}).get('@MetodoPago',{}),
            #'uso_cfdi':receptor_data.get('@UsoCFDI'),
...
#                         tax_ids.append(tax_exist.id)
        ...
        result['views'] = [(res and res.id or False, 'form')]
        result['res_id'] = invoice_exist.id
        self.attachment_id.write({'creado_en_odoo':True, 'invoice_ids':[(6,0, [vendor_invoice.id])], 'res_id': vendor_invoice.id, 'res_model': vendor_invoice._name,})
        return result
...
#                     tax_ids.append(tax_exist.id)
    ...
    @api.model
    def get_or_create_product(self, default_code, product_name, unit_price):
        product_exist = False
        product_obj = self.env['product.product']
        if default_code:
            product_exist = product_obj.search([('default_code','=',default_code)],limit=1)
...
#                         tax_ids.append(tax_exist.id)
            ...
            product_exist = self.get_or_create_product(default_code, product_name, unit_price)
            if discount_amount:
                discount_percent = discount_amount*100.0/(unit_price*qty)
            else:
                discount_percent=0.0
    ...
    @api.multi
    def import_purchase_order(self, file_contents):
        file_content =file_contents.datas
        file_content = base64.b64decode(file_content)
        file_content = file_content.replace(b'cfdi:',b'')
        file_content = file_content.replace(b'tfd:',b'')
        ...
        #vendor = self.env['import.purchase.order.from.xml'].create_update_vendor(vendor_data)
        #raise Warning(file_contents.res_id)
        vendor = self.create_update_partner(vendor_data, is_supplier=True)
        order_vals = {
            'partner_id':vendor.id,
            'partner_ref':vendor_reference,
            'l10n_mx_edi_cfdi_uuid_cusom':purchase_uuid,
            'l10n_mx_edi_cfdi_name': file_contents.datas,
            'state': 'purchase',
            #'forma_pago':data.get('Comprobante',{}).get('@FormaPago',{}), 
            #'methodo_pago':data.get('Comprobante',{}).get('@MetodoPago',{}),
            #'uso_cfdi':receptor_data.get('@UsoCFDI'),
        ...
        #msg = file_contents
        #txt = _('XML Imported')
        #purchase_order_exist.message_post(attachment_ids=msg, body=txt)
        #search_code = 'Propio'
        #p_supplierinfo = self.env['product.supplierinfo']
        for line in order_line_data:
            product_name = line.get('@Descripcion')
